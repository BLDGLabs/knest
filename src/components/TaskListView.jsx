          ...
          <div className="space-y-1">
            {filteredAndSorted.map(task => {
              ...
              return (
                <div
                  key={task.id}
                  className={`border-l-2 ${priorityConfig.color} hover:bg-dark-card/30 transition-colors cursor-pointer`}
                  onClick={() => onTaskClick(task)}
                  style={{ backgroundColor: 'transparent' }}
                >
                  <div className="px-3 py-2 flex items-center gap-3" style={{ backgroundColor: 'transparent' }}>
                    {/* Checkbox */}
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
